...
const TilePattern = ({ suit, value, size }: { suit: TileSuit, value: TileValue, size: 'md' | 'sm' }) => {
    ...
    if (suit === 'dots') {
        ...
        switch (value) {
            ...
            case '3': return <div className="w-full h-full grid grid-cols-3 grid-rows-3 p-1"><div className="col-start-1 row-start-1 self-start justify-self-start">{dots(1, redColor)}</div><div className="col-start-2 row-start-2 self-center justify-self-center">{dots(1, dotColor)}</div><div className="col-start-3 row-start-3 self-end justify-self-end">{dots(1, greenColor)}</div></div>;
            case '4': return <div className="w-full h-full flex justify-center items-center gap-4 p-1"><div className="flex flex-col gap-3">{dots(1, blueColor)}{dots(1, greenColor)}</div><div className="flex flex-col gap-3">{dots(1, greenColor)}{dots(1, blueColor)}</div></div>;
            ...
            case '7': return <div className="w-full h-full flex flex-col justify-center items-center gap-1 p-1"><div className="flex justify-between w-full -mb-1"><div className="translate-x-1">{dots(1, greenColor)}</div><div>{dots(1, greenColor)}</div><div className="-translate-x-1">{dots(1, greenColor)}</div></div><div className="flex justify-around w-full">{dots(2, redColor)}</div><div className="flex justify-around w-full">{dots(2, redColor)}</div></div>;
    ...
    if (suit === 'characters') {
        return <div className="relative flex flex-col items-center justify-center h-full p-1 font-bold font-headline leading-none"><span className={cn('text-black', size === 'md' ? 'text-lg' : 'text-base')}>{symbolMap[value]}</span><span className={cn('text-red-600', baseFontSize)}>{characterMap[value]}</span></div>;
    }
    if (suit === 'wind') {
