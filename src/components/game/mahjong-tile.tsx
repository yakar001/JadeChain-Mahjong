...
const TilePattern = ({ suit, value, size }: { suit: TileSuit, value: TileValue, size: 'md' | 'sm' }) => {
    ...
    if (suit === 'dots') {
        ...
        switch (value) {
            case '1': return <div className={cn("w-10 h-10 flex justify-center items-center", size === 'sm' && "w-8 h-8")}><svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="40" fill="#d1e4c1" stroke="#508a38" strokeWidth="2" /><circle cx="50" cy="50" r="30" fill="none" stroke="#508a38" strokeWidth="1" strokeDasharray="4" /><circle cx="50" cy="50" r="22" fill="#f4c3c3" stroke="#c04848" strokeWidth="1.5" /><circle cx="50" cy="50" r="14" fill="#f4c3c3" /><path d="M50,15 A35,35 0 0,1 50,85 A35,35 0 0,1 50,15 M35,25 A25,25 0 0,1 65,25 A25,25 0 0,1 35,25 M25,35 A15,15 0 0,1 75,35 A15,15 0 0,1 25,35" fill="none" stroke="#508a38" strokeWidth="0.5" opacity="0.5" /></svg></div>;
            ...
            case '4': return <div className="w-full h-full flex justify-around items-center p-1"><div className="flex flex-col gap-3">{dots(1, blueColor)}{dots(1, greenColor)}</div><div className="flex flex-col gap-3">{dots(1, greenColor)}{dots(1, blueColor)}</div></div>;
            ...
            case '6': return <div className="w-full h-full flex flex-col justify-around p-1"><div className="flex justify-around">{dots(2, redColor)}</div><div className="flex justify-around">{dots(4, greenColor)}</div></div>;
            case '7': return <div className="w-full h-full flex flex-col justify-around p-1 -space-y-1"><div className="flex justify-between w-full px-1"><div className="translate-y-1">{dots(1, greenColor)}</div><div className="-translate-y-1">{dots(1, greenColor)}</div></div><div className="flex justify-center -translate-y-1">{dots(1, greenColor)}</div><div className="flex justify-around">{dots(2, redColor)}</div><div className="flex justify-around">{dots(2, redColor)}</div></div>;
    ...
    if (suit === 'bamboo') {
        ...
        switch (value) {
            case '1': return <svg className={cn("w-10 h-10", size === 'sm' && "w-8 h-8")} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><path d="M50 8C35 15, 30 38, 50 45 C70 38, 65 15, 50 8Z" fill="#508a38" /><path d="M50 43C40 50, 42 65, 50 70 C58 65, 60 50, 50 43Z" fill="#508a38" /><path d="M48 68 C35 75, 35 90, 48 95 L52 95 C65 90, 65 75, 52 68Z" fill="#c04848" /><path d="M50 44 C52 46, 52 49, 50 51 C48 49, 48 46, 50 44Z" fill="white" /><path d="M42 22 C40 20, 38 22, 40 25 C45 28, 55 28, 60 25 C62 22, 60 20, 58 22 C55 24, 45 24, 42 22Z" fill="#508a38" /></svg>;
            ...
            case '4': return <div className="w-full h-full flex flex-col justify-around items-center p-1"><div className="flex justify-center gap-2">{sticks(1, blueColor)}{sticks(1, greenColor)}</div><div className="flex justify-center gap-2">{sticks(1, greenColor)}{sticks(1, blueColor)}</div></div>;
            case '5': return <div className="w-full h-full flex flex-col justify-around p-1"><div className="flex justify-around">{sticks(2, bambooColor)}</div><div className="flex justify-center">{sticks(1, redColor)}</div><div className="flex justify-around">{sticks(2, bambooColor)}</div></div>;
            case '6': return <div className="w-full h-full flex flex-col justify-around"><div className="flex justify-around">{sticks(3, greenColor)}</div><div className="flex justify-around">{sticks(3, blueColor)}</div></div>;
            ...
            case '8': return <div className="w-full h-full flex flex-col justify-center items-center gap-1"><div className="flex gap-4"><div className={cn(sizeClass.bamboo, "rotate-45", greenColor)}/><div className={cn(sizeClass.bamboo, "-rotate-45", greenColor)}/></div><div className="flex gap-2"><div className={cn(sizeClass.bamboo, "-rotate-45", blueColor)}/><div className={cn(sizeClass.bamboo, "rotate-45", blueColor)}/></div><div className="flex gap-4"><div className={cn(sizeClass.bamboo, "rotate-45", greenColor)}/><div className={cn(sizeClass.bamboo, "-rotate-45", greenColor)}/></div></div>;
    ...
    if (suit === 'characters') {
        return <div className="relative flex flex-col items-center justify-start h-full p-1 font-bold font-headline leading-none"><span className={cn('text-black', size === 'md' ? 'text-lg' : 'text-base')}>{symbolMap[value]}</span><span className={cn('text-red-600 absolute bottom-1', baseFontSize)}>{characterMap[value]}</span></div>;
    }
    if (suit === 'wind') {
